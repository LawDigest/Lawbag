...
public class CongressmanService {
    ...
    public Map<String, Object> getCongressmanDetails(String congressmanId, Pageable pageable) {
        ...
        // Get bill IDs where the congressman is the representative proposer
        List<String> representativeBillIds = billProposerRepository.findRepresentativeBillIdsByCongressmanId(congressmanId);

        // Apply pagination to the list of bill IDs
        int start = (int) pageable.getOffset();
        int end = Math.min((start + pageable.getPageSize()), representativeBillIds.size());
        List<String> pagedBillIds = representativeBillIds.subList(start, end);

        List<CongressDetailBillDto> detailedBills = buildDetailedBillDtos(pagedBillIds);

        // Calculate if it's the last page
        boolean isLastPage = end >= representativeBillIds.size();
        int pageNumber = pageable.getPageNumber();

        PaginationResponse paginationResponse = new PaginationResponse(isLastPage, pageNumber);
...
//    }
    private CongressDetailBillDto buildDetailedBillDto(Bill bill, BillProposer representativeProposer, List<BillProposer> proposers) {
        // Fetch party details for the representative proposer
